...
# 导入 X_RANGE
from mini_bdx_runtime.mini_bdx_runtime.xbox_controller import X_RANGE
...
class Tools:
    ...
    def move_forward(self, seconds=2):
        ...
        # 使用与手柄控制相同的前进速度计算逻辑
        lin_vel_x = 1.0  # 模拟手柄左摇杆垂直轴最大向前输入
        if lin_vel_x >= 0:
            lin_vel_x *= np.abs(X_RANGE[1])
        self.rl_walk.last_commands[0] = lin_vel_x
        time.sleep(seconds)
    ...
    def move_backward(self, seconds=2):
        ...
        lin_vel_x = -1.0  # 模拟手柄左摇杆垂直轴最大向后输入
        if lin_vel_x < 0:
