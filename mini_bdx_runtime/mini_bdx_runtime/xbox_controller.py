...
class XBoxController:
    def __init__(self, command_freq, only_head_control=False):
        ...
        # 新增Y键长按检测变量
        self.y_press_start = None  # 记录Y键按下的起始时间
        self.y_hold_duration = 0.5  # 切换所需的长按时间(秒)
        self.y_toggled = False      # 标记本次按压是否已完成切换
    ...
    def get_commands(self):
        ...
        # Y键长按检测逻辑
        if self.Y_pressed and self.y_press_start is not None and not self.y_toggled:
            # 计算已按压时间
            press_time = current_time - self.y_press_start
            # 满足长按时间且非仅头控模式，执行切换
            if press_time >= self.y_hold_duration and not self.only_head_control:
                self.head_control_mode = not self.head_control_mode
                self.y_toggled = True  # 标记为已切换，防止持续按压重复切换
